...
self.microns_per_pixel_imagingXY = (
        self.MICRONS_PER_PIXEL_ZSTACK_XY *
        self.ZSTACK_TO_IMAGING_RESOLUTION_DIFF
)
...
def get_atlas_coordinates(self) -> np.ndarray:
    ...
    def get_transformed_coordinates(self) -> Optional[np.ndarray]:
        """Load pre-transformed coordinates if they exist."""
        coord_file = Path(lisabasedatapath) / f'{self.animalnum}' / 'transformed_coords.npy'
        if coord_file.exists():
            return np.load(coord_file)
        logger.warning(f"No transformed coordinates found for animal {self.animalnum}")
        return None
...
def cellpose_model_ops():
    ...
    return {
        ...
        'bruker_bidirectional': False,  # bidirectional multiplane in bruker: 0, 1, 2, 2, 1, 0 (True) vs 0, 1, 2, 0,
        # 1, 2 (False)
        ...
        'move_bin': False,  # if 1, and fast_disk is different from save_disk, binary file is moved to save_disk

        # main settings
        'nplanes': 1,  # each tiff has these many planes in sequence
        'nchannels': 1,  # each tiff has these many channels per plane
        'functional_chan': 1,  # this channel is used to extract functional ROIs (1-based)
        'tau': 3.,  # this is the main parameter for deconvolution
        ...
        'bidiphase': 0,  # Bidirectional Phase offset from line scanning (set by user). Applied to all frames in
        # recording.
        ...
        # whether or not to run registration twice (useful for low SNR data). Set keep_movie_raw to True if setting
        # this parameter to True.
        'keep_movie_raw': False,  # whether to keep binary file of non-registered frames.
        'nimg_init': 300,  # subsampled frames for finding reference image
        'batch_size': 500,  # number of frames per batch
        'maxregshift': 0.1,  # max allowed registration shift, as a fraction of frame max(width and height)
        'align_by_chan': 1,  # when multichannel, you can align by non-functional channel (1-based)
        ...
        'th_badframes': 1.0,  # this parameter determines which frames to exclude when determining cropping - set it
        # smaller to exclude more frames
        ...
        'block_size': [64, 64],  # block size to register (** keep this a multiple of 2 **)
        'snr_thresh': 1.2,  # if any nonrigid block is below this threshold, it gets smoothed until above this
        # threshold. 1.0 results in no smoothing
        ...
        'spatial_taper': 40,  # how much to ignore on edges (important for vignetted windows, for FFT padding do not
        # set BELOW 3*ops['smooth_sigma'])
        ...
        'anatomical_only': 4,  # run cellpose to get masks on 1: max_proj / mean_img; 2: mean_img; 3: mean_img
        # enhanced, 4: max_proj
        'diameter': 6,  # use diameter for cellpose, if 0 estimate diameter
        'cellprob_threshold': -1,  # cellprob_threshold for cellpose
        'flow_threshold': 1.0,  # flow_threshold for cellpose
        'spatial_hp_cp': 0,  # high-pass image spatially by a multiple of the diameter
        'pretrained_model': '2P_model',  # path to pretrained model or model type string in Cellpose (can be user model)

        # classification parameters
        'soma_crop': True,  # crop dendrites for cell classification stats like compactness ROI extraction parameters
        'neuropil_extract': True,  # whether or not to extract neuropil; if False, Fneu is set to zero
        'inner_neuropil_radius': 2,  # number of pixels to keep between ROI and neuropil donut
        'min_neuropil_pixels': 350,  # minimum number of pixels in the neuropil
        'lam_percentile': 50.,  # percentile of lambda within area to ignore when excluding cell pixels for neuropil
        # extraction
